...
def _row_to_user(row) -> UserResponse:
    return UserResponse(**{k: row[k] for k in ("id", "name", "email", "company_name", "created_at")})
...
def register(payload: RegisterRequest, db: sqlite3.Connection = Depends(get_db)) -> TokenResponse:
    ...
    try:
        db.execute(
            "INSERT INTO users (id, name, email, company_name, password_hash, created_at) VALUES (?, ?, ?, ?, ?, ?)",
            (user_id, payload.name.strip(), payload.email.lower(), payload.company_name, pw_hash, now),
        )
        db.commit()
...
def me(current_user: dict = Depends(get_current_user)) -> UserResponse:
    return UserResponse(**{k: current_user[k] for k in ("id", "name", "email", "company_name", "created_at")})
